    <div class="content">
        ...
        <div class="working_area">
			...
			<?php
				...
				//ACTION STATEMENT
				$action = HTTP_PATH ."users/voters_department/insert";
				...
				if(isset($id))
				{
					$action = HTTP_PATH ."users/voters_department/update/".$id;
					$sql = $CI->db->query("SELECT department_name FROM voters_department WHERE id= $id");
			...
			<!-- STATUS TABLE  -->
			<form name="SMBi2" id="statusTable" action="<?php echo HTTP_PATH ?>users/voters_department/deleteSelectedItem" method="POST"> 
				<?php echo $csrf ?>
				 <div class="clear"></div>
					...
					<?php 
						...
						foreach($departments as $d)
						{
							...
							echo "<tr>";
								...
								if($EDIT)
									echo "<a href='".HTTP_PATH."users/voters_department/edit/".$id."'>Edit</a> |"; 
								if($DELETE)	
	...
	<?php if($last>0){ ?>
		<ul class="pagination">
				<a href="<?php echo HTTP_PATH."users/voters_department/page/1" ?>"><li class="page-btn" style="margin-left:3px;"> &laquo; FIRST </li></a> 
				<?php 
					//PAGNINATION
					$i	   = 1; 
					$page  = 1;
					...
					while($l!=0)
					{
						...
						//ACTIVE PAGE
						$style="";
						$page_link = HTTP_PATH."users/voters_department/page/".$i++;
						if($active_page==$page++)
						{
				...
				?>
				<a href="<?php echo HTTP_PATH."users/voters_department/page/".$last ?>"><li class="page-btn">LAST &raquo;</li></a>
		</ul>
	<?php } ?>
    </div>
	...
	<script type="text/javascript">
		function deleteOneItem(id)
		{
			jConfirm("Are you sure you want to delete this Department?","Alert",function(r){
				if(r) window.location = "<?php echo HTTP_PATH ?>users/voters_department/deleteOneItem/"+ id;
			});
		}
