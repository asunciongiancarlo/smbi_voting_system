    <div class="content">
        ...
        <div class="working_area">
			...
			<?php
				...
				//ACTION STATEMENT
				$action = HTTP_PATH ."itemDatabase/OUTLET_status/insert";
				...
				if(isset($id))
				{
					$action = HTTP_PATH ."itemDatabase/OUTLET_status/update/".$id;
					$sql = $CI->db->query("SELECT statusName FROM OUTLET_Status WHERE id= $id");
			...
			if($ADD OR $EDIT){
				...
				echo $CI->forms->form_fields('text','statusName',$statusNameVal,'ADD OUTLET TYPE NAME','r');
			...
			<!-- STATUS TABLE  -->
			<form name="SMBi2" id="statusTable" action="<?php echo HTTP_PATH ?>itemDatabase/OUTLET_status/deleteSelectedItem" method="POST"> 
				<?php echo $csrf ?>
				<div class="clear"></div>
				<table cellpadding="0" cellspacing="0" style="width:100%;margin: 0px auto;">
					...
					<tr style="border-radius: 6px;">
						...
						<th style="width:200px;text-align:center;"> STATUS NAME </th> 
					...
					<?php 
						...
						foreach($status as $s)
						{
							...
							echo "<tr>";
								...
								if($EDIT)
									echo "<a href='".HTTP_PATH."itemDatabase/OUTLET_status/edit/".$id."'>Edit</a> |"; 
								if($DELETE)	
	...
	<?php if($last>0){ ?>
		<ul class="pagination">
				<a href="<?php echo HTTP_PATH."itemDatabase/OUTLET_status/page/1" ?>"><li class="page-btn" style="margin-left:3px;"> &laquo; FIRST </li></a> 
				<?php 
					//PAGNINATION
					$i	   = 1; 
					$page  = 1;
					...
					while($l!=0)
					{
						...
						//ACTIVE PAGE
						$style="";
						$page_link = HTTP_PATH."itemDatabase/OUTLET_status/page/".$i++;
						if($active_page==$page++)
						{
				...
				?>
				<a href="<?php echo HTTP_PATH."itemDatabase/OUTLET_status/page/".$last ?>"><li class="page-btn">LAST &raquo;</li></a>
		</ul>
	<?php } ?>
    </div>
	...
	<script type="text/javascript">
		...
		{
			jConfirm("Are you sure you want to delete this Type of Outlet?","Alert",function(r){
				if(r) window.location = "<?php echo HTTP_PATH ?>itemDatabase/OUTLET_status/deleteOneItem/"+ id;
			});
		}
		
		function deleteSelectedItem()
		{
			jConfirm("Are you sure you want to delete Multiple Item Type?","Alert",function(r){
