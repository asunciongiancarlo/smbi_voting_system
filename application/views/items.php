<div class="content" style='padding-left:1px;'>
	<div class="title-content" style='width:100%;'>
		<h2 class="item-galla" style='float:left;'>ITEM DATABASE</h2>
		
		<div style='float:right;margin-right:58px;'>
			<div class="fl search_tools items-searchtools">
				<div class="additem" >
					<?php if($ADD & $_SESSION['super_admin']!='y'){ ?>
					<a href="<?php echo HTTP_PATH ."itemDatabase/items/add" ?>"> 
						<img class="max-add" src='<?php echo HTTP_PATH.'img/add_item.png'; ?>'>
						<img class="min-add" src='<?php echo HTTP_PATH.'img/add.png'; ?>'>
					</a>
					<?php } ?>
				</div>
	...
	<div>
		...
		<div  style="font-weight:bold; font-size:15px;" class="container">
			...
			?>
			<div class="row1">
			<form name="SMBi2" id="itemsForm" action="<?php echo HTTP_PATH ?>itemDatabase/tag_for_purging/items/purgeSelectedItems" method="POST" class="item-container" style="margin-left: 18px;margin-top: 14px;"> 			
			<?php 		
				if(!empty($data) & $_SESSION['super_admin']!='y' & ($DELETE OR $TAG_AS_POPULAR OR $ADD_TO_ARCHIVE)){
			...
			?>
				<p style="font-weight:bold;cursor:pointer;font-size:12px;margin:10px 0 0 10px;width:180px;background:#370909;color:white;" onclick="deleteSelectedItem()" class="buttonWithRadius fl">  
					<img src="<?php echo HTTP_PATH ."img/delete.png"?>" style="margin-right:10px;height: 14px;">PURGE SELECTED ITEMS 
				</p>
				<?php } if($TAG_AS_POPULAR){ ?>	
					<p style="font-weight:bold;cursor:pointer;font-size:12px;margin:10px 0 0 10px;width:176px;background:#370909;color:white;" onclick="SetAsPopularItems()" class="buttonWithRadius fl">  
						<img src="<?php echo HTTP_PATH ."img/set-popular.png"?>" style="margin-right:10px;height: 13px;">TAG AS POPULAR ITEMS 
					</p> 
			...
			<?php
				...
				echo $csrf;
					...
					$w = w($item_img);
					?>
					<div class="common-gal fl items-item" style="border:1px solid #999999;height:299px;overflow:hidden;">
						...
						<div class="clear"></div>
						 ...
						 if(campaign_items($itemID)!=TRUE){
								if($ADD & $_SESSION['super_admin']!='y')
									echo "<a href='".HTTP_PATH."itemDatabase/items/duplicate/".$itemID."'>Duplicate</a> | ";
								if(($EDIT & $_SESSION['super_admin']!='y') OR ($VENDORS_EDIT & $_SESSION['super_admin']!='y'))	
									echo "<a href='".HTTP_PATH."itemDatabase/items/edit/".$itemID."/item_db'>Edit</a> | ";
								if($DELETE & $_SESSION['super_admin']!='y')	
									echo "<a style='cursor:pointer' onclick='deleteOneItem($itemID)'>Purge</a> | ";
								if($TAG_AS_POPULAR & $_SESSION['super_admin']!='y' & $popular=='n')	
									echo "<a style='cursor:pointer' onclick='setAsPopularOneItem($itemID)'>Popular</a> | ";
						...
						echo "</label>";
						 ...
						 if($DELETE & $_SESSION['super_admin']!='y' & campaign_items($itemID)!=TRUE){
	   ...
	   function SetAsNotPopularItems()
		{
			jConfirm("Remove from popular items?","Alert",function(r){
				if(r){
					$("#itemsForm").attr('action','<?php echo HTTP_PATH ."itemDatabase/SetAsNotPopularItems/items" ?>');
		...
		function setAsNotPopularOneItem(id)
		{
			jConfirm("Set this as not popular item?","Alert",function(r){
				if(r){
					window.location = "<?php echo HTTP_PATH ?>itemDatabase/SetAsNotPopularItems/items/"+ id;
				}
			});
		}
		
		function setAsPopularOneItem(id)
		{
			jConfirm("Set this as popular item?","Alert",function(r){
				if(r) window.location = "<?php echo HTTP_PATH ?>itemDatabase/SetAsPopularItems/"+ id;
			});
		}
		...
		function deleteOneItem(id)
		{
			jConfirm("Are you sure you want to submit this item for purging?","Alert",function(r){
				if(r) window.location = "<?php echo HTTP_PATH ?>itemDatabase/tag_for_purging/items/purgeOneItem/"+ id;
			});
		}
		...
		function archiveOneItem(id)
		{
			jConfirm("Are you sure you want to submit this item for archiving?","Alert",function(r){
				if(r) window.location = "<?php echo HTTP_PATH ?>itemDatabase/tag_for_archiving/items/archiveOneItem/"+ id;
			});
		}
		...
		function archiveSelectedItem()
		{
			jConfirm("Are you sure you want to submit these items for archiving?","Alert",function(r){
				$("#itemsForm").attr('action','<?php echo HTTP_PATH ."itemDatabase/tag_for_archiving/items/archiveSelectedItems" ?>');
				if(r) document.getElementById("itemsForm").submit();
			});
		}
