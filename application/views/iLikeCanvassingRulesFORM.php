<div class="content">
	...
	<div class="working_area">
		...
		$action = HTTP_PATH.'users/iLikeCanvassingRules/insert';
		$CI2 =& get_instance();
		$CI2->load->library('fv');
		
		if(isset($iLikeCanvassingRulesID)){
			$sql = "SELECT * FROM iLikeCanvassingRules WHERE iLikeCanvassingRules.id = $iLikeCanvassingRulesID";
			$sql = $CI->db->query($sql);
			$iLikeCanvassingRules = $sql->row();
			
			$countryID = $iLikeCanvassingRules->countryID;
			$fieldName = $iLikeCanvassingRules->fieldName;
			$fieldID   = $iLikeCanvassingRules->fieldID;
			$POSMTypeID= $iLikeCanvassingRules->fieldID;
			$price_rangeID = $iLikeCanvassingRules->price_rangeID;
			$val 	   = $iLikeCanvassingRules->val;
			$rel 	   = $iLikeCanvassingRules->rel;
			$cond1 			  = $iLikeCanvassingRules->cond1;
			$min_val 		  = $iLikeCanvassingRules->min_val;
			$logical_operator = $iLikeCanvassingRules->logical_operator;
			$cond2   		  = $iLikeCanvassingRules->cond2;
			$max_val 		  = $iLikeCanvassingRules->max_val;
			$action = HTTP_PATH."users/iLikeCanvassingRules/update/$iLikeCanvassingRulesID";
		}
...
<?php 
	...
	echo $CI->forms->form_header('SMBi','iLikeCanvassingRules',$action);
...
?>
		...
		<div class='fl  searhPanel' style='width:170%;margin-top:-20px;'>
			  ...
			  <select name='countryID' style='width:25%;margin-right:10px;' disabled>
			   ...
			   ?>
			   <!-- POSMTypeID -->
			   <?php if(!isset($iLikeCanvassingRulesID) OR (isset($iLikeCanvassingRulesID) AND $fieldName=='POSMTypeID')){ ?>
			   ...
			   <select name='cond1' class='fl' style='width:9%;margin-right:26px;'>
				...
				<?php
				if(isset($POST) AND !isset($iLikeCanvassingRulesID)){ 
				extract($POST);
				$val 	 = "";
				$fieldID = "";
				$rel     = "";
