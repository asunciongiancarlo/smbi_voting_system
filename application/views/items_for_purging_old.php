<div class="content" style='padding-left:1px;'>
	<div class="title-content" style='width:100%;'>
		<h2 class="item-galla" style='float:left;'>ITEMS FOR PURGING</h2>
	...
	<div>
		...
		<div  style="font-weight:bold; font-size:15px; padding-top:20px;" class="container">
			<?php 
				$CI =& get_instance();
			...
			?>
			<div class="row1">
			<form name="SMBi2" id="itemsForm" action="<?php echo HTTP_PATH ?>itemDatabase/items_for_purging/deleteSelectedItem" method="POST" class="item-container" style="margin-left: 18px;"> 			
			<?php 		
				if(!empty($data) & $_SESSION['super_admin']!='y'){
					if($DELETE OR $RESTORE) echo "<label style='margin:-10px 0 0 10px;'> <input type='checkbox' onclick='checkedAll()' style='vertical-align:top;color:#555555'> SELECT ALL </label>";
				
				if($DELETE){ ?>
				<p style="font-weight:bold;cursor:pointer;font-size:12px;margin:10px 0 0 10px;width:270px;background:#370909;color:white;" onclick="deleteSelectedItem()" class="buttonWithRadius fl">  
					<img src="<?php echo HTTP_PATH ."img/delete.png"?>" style="margin-right:10px;">DELETE PERMANENTLY SELECTED ITEMS 
				</p>
				<?php } if($RESTORE){ ?>
				<p style="font-weight:bold;cursor:pointer;font-size:12px;margin:10px 0 0 10px;width:192px;background:#370909;color:white;" onclick="restoreSelectedItem()" class="buttonWithRadius fl">  
				...
				echo $csrf;
					...
					<div class="common-gal fl items-item" style="border:1px solid #999999;min-height:299px;overflow:hidden;">
						...
						<div class="common-gal-heading" style='text-align:center;height:25px;color:white;font-style:bold;margin-bottom:3px;<?php echo $background ?>' <?php echo $title ?>> 
							...
							<?php if($publish!='y' AND $irrelevant=='n') echo "<br/><label style='font-size:11px;color:red;margin-top:-3px;'>*For Approval</label>"; 
								  elseif($publish!='y' AND $irrelevant=='y')  echo "<br/><label style='font-size:11px;color:red;margin-top:-3px;'>*Disapproved</label>"; 
							?>
						</div>
						<div class="clear"></div>
						 ...
						 if(campaign_items($itemID)!=TRUE){
								if(($EDIT & $_SESSION['super_admin']!='y'))	
									echo "<a href='".HTTP_PATH."itemDatabase/items_for_purgingFORM/edit/".$itemID."/purge'>Edit</a> | ";
								if(($DELETE & $_SESSION['super_admin']!='y'))	
									echo "<a onclick='deleteOneItem($itemID)'>Delete</a> ";
						}else{
							echo "Campaign Item: Edit and Delete are not available.";
						}
						echo "</label>";
						
						 if(($DELETE OR $RESTORE) & $_SESSION['super_admin']!='y'){
	...
	<script type="text/javascript">
		...
		function deleteOneItem(id)
		{
			jConfirm("Permanently delete this item?","Alert",function(r){
				if(r) window.location = "<?php echo HTTP_PATH ?>itemDatabase/items_for_purging/deleteOneItem/"+ id;
			});
		}
		...
		{
			jConfirm("Are you sure you want to submit these multiple Items to purging?","Alert",function(r){
		...
		function restoreSelectedItem()
		{
			jConfirm("Are you sure you want to restore these Items?","Alert",function(r){
				if(r){ 
				document.getElementById("itemsForm").action = "<?php echo HTTP_PATH ?>itemDatabase/items_for_purging/restoreSelectedItem"
				document.getElementById("itemsForm").submit();
				}
			});
		}
