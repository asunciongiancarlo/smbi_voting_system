<div class="content">
	...
	<div class="working_area">
		...
		<div class='fl  searhPanel' style='width:100%!important;margin-top:-20px;'>
		      <form name='BU_activeness' action='<?php echo HTTP_PATH ."item_distribution_Preview/$view/$countryID/$month/$year/$fld/$fld_val"; ?>' method='post'>
				  ...
				  <h2 class='fl form' style='width:50%;margin-right:10px;'> CATALOGUE: <?php echo $ecTitle; ?></h2>
			   ...
			   <div style='clear:both;'></div>
				...
				<tbody>
				 <?php 
					...
					foreach($rows as $r)
					{extract($r);
					$sub_total += $num_ec_items; 
					$c = (($x++)%2) == 0 ? "class='alter'" :  ""; 
					$col = ($col=='') ? 'Uncategorized' : $col;
				 ?>
				<tr>
				  <td>												<?php echo $x     ?> 			</td>
				  ...
				  <td style='text-align:center;'>
					<!--($view='',$countryID='',$month='',$year='',$fld='',$fld_val='') -->
					 <?php echo $num_ec_items  ?> 
				  </td>
				  <?php if($previewType=='default'){ ?>
					<td><a href="<?php echo HTTP_PATH.'report/ecitem_distribution_Preview/'.$previewType.'/'.$ecID.'/null/null/'.$fld.'/'.$fieldValue ?>" style='cursor:pointer;'> View Items  </a></td>
				  <?php }elseif($previewType=='gYear'){ ?>
					<td><a href="<?php echo HTTP_PATH.'report/ecitem_distribution_Preview/'.$previewType.'/'.$ecID.'/null/'.$year.'/'.$fld.'/'.$fieldValue ?>" style='cursor:pointer;'> View Items  </a></td>
				  <?php }elseif($previewType=='gMonth'){ ?>
					<td><a href="<?php echo HTTP_PATH.'report/ecitem_distribution_Preview/'.$previewType.'/'.$ecID.'/'.$mID.'/'.$year.'/'.$fld.'/'.$fieldValue ?>" style='cursor:pointer;'> View Items  </a></td>
				  <?php } ?>
				</tr>
				 <?php }
				 echo "<tr>
