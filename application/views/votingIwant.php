	    ...
	    <tr>
	     ...
	     <td class="td-stay"> 
			...
			Please vote on all the items below. You may either click 
             <input value="Want"  style='cursor: pointer;font-size: 11px;height: 22px;padding: 0;'   type="button" class="like-btn" alt="">  
			 or
 ...
 <div class="processBar" align="center">
    <?php
	  ...
	  { 
		 ...
		 $path =isset($_SESSION['itemReview'.$encemail])? HTTP_PATH. "gallery/iWant/$cam_id/$email/1/$k/0":"#";
         //var_dump($path);  	   
	   ?>
	   ...
	   <?php 
		 ...
		 ?>
		<a href="<?php echo $_SESSION['itemReview'.$encemail]==true ?  HTTP_PATH. "gallery/iWantreview/$cam_id/$email":"#"?>"> 
	...
	<?php
	 $keyOfActive ='';// $curLvl="";
	  ...
	  if(isset($priceRange[$keyOfActive+1]['id'])==false){
  	     $nextLvlpath =  HTTP_PATH. "gallery/iWant/$cam_id/$email/1/".($curIndexPOSM+1)."/0";
		 if($priceRange[$keyOfActive-1]['id']==false)
		 {
		 $prevLvlpath =  HTTP_PATH. "gallery/iWant/$cam_id/$email/1/".($curIndexPOSM-1)."";
		 }else{
  	     $prevLvlpath =  HTTP_PATH. "gallery/iWant/$cam_id/$email/1/".($curIndexPOSM)."/".($priceRange[$keyOfActive-1]['id'])."";;  
		 }
	  }else{
	     $nextLvlpath =  HTTP_PATH. "gallery/iWant/$cam_id/$email/1/".($curIndexPOSM)."/".($priceRange[$keyOfActive+1]['id'])."";
		 
		 if($priceRange[($keyOfActive-1)]['id']==false)
          $prevLvlpath =  HTTP_PATH. "gallery/iWant/$cam_id/$email/1/".($curIndexPOSM-1)."/".(($priceRange[($keyOfActive)]['id'])-1)."";
		 else{
		  $prevLvlpath =  HTTP_PATH. "gallery/iWant/$cam_id/$email/1/".($curIndexPOSM)."/".($priceRange[($keyOfActive-1)]['id'])."";
		 }
	...
	if(is_array($items)) { 
		 ...
		 <div class="clear"></div>
				<?php
				   ...
				   if( $curIndexPOSM ==  ($totalPOSMTYPE -1) and ($keyOfActive== $totalPRICERANGE-1)) $nextLvlpath	= HTTP_PATH.    "gallery/iWantreview/$cam_id/$email"; 
				   $frwURL    = $nextLvlpath;
				  
				   $backbtn 	=  ($curIndexPOSM>0 or $keyOfActive >0) ? true:false;   
				   $submitURL = HTTP_PATH .    "gallery/finish/$cam_id/$email";
				   $reloadURL = HTTP_PATH .   "gallery/iWantreview/$cam_id/$encemail";
				?>
