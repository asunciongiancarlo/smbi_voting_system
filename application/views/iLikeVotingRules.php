<div class="content">
		...
		<?php
			...
			{extract($country);
				...
				{ extract($p_type);
				  ...
				  $sql = $CI->db->query("SELECT COUNT(id) as num_of_levels_present FROM iLikeVotingRules WHERE fieldID = $pID AND countryID = $cID");
				  $sql = $sql->row();
				 
				  $img = "";
				  if($ADD AND ($sql1->num_of_level!=$sql->num_of_levels_present)) 
								$img = "<a href='".HTTP_PATH."users/iLikeVotingRules/add/$cID/$pID' class='add-voting2' style='color:white;'>
				  ...
				  { extract($s);
					...
					if($EDIT)
						echo "<a href='".HTTP_PATH."users/iLikeVotingRules/edit/$iLikeVotingRulesID'>Edit</a> | ";
					if($DELETE)	
						echo "<a style='cursor:pointer' onclick='deleteOneItem($iLikeVotingRulesID)'>Delete</a> </td>";
					echo "</tr>";
				  }
				}
	...
	<?php if($last>0){ ?>
		<ul class="pagination">
		<a href="<?php echo HTTP_PATH."users/iLikeVotingRules/page/1" ?>"><li class="page-btn" style="margin-left:3px;"> &laquo; FIRST </li></a> 
		<?php 
			//PAGNINATION
			$i	   = 1; 
			$page  = 1;
			...
			while($l!=0)
			{
				...
				//ACTIVE PAGE
				$style="";
				$page_link = HTTP_PATH."users/iLikeVotingRules/page/".$i++;
				if($active_page==$page++)
				{
		...
		?>
		<a href="<?php echo HTTP_PATH."users/iLikeVotingRules/page/".$last ?>"><li class="page-btn">LAST &raquo;</li></a>
		</ul>
	<?php } ?>
    </div> 
...
<script type="text/javascript">
	function deleteOneItem(id)
	{
		jConfirm("Are you sure you want to delete this iLike Voting Rule?","Alert",function(r){
			if(r) window.location = "<?php echo HTTP_PATH ?>users/iLikeVotingRules/deleteOneItem/"+ id;
		});
