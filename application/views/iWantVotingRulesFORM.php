<div class="content">
	...
	<div class="working_area">
		...
		$action = HTTP_PATH.'users/iWantVotingRules/insert';
		$CI2 =& get_instance();
		$CI2->load->library('fv');
		
		if(isset($iWantVotingRulesID)){
			$sql = "SELECT * FROM iWantVotingRules WHERE iWantVotingRules.id = $iWantVotingRulesID";
			$sql = $CI->db->query($sql);
			$iWantVotingRules = $sql->row();
			
			$countryID = $iWantVotingRules->countryID;
			$fieldName = $iWantVotingRules->fieldName;
			$fieldID   = $iWantVotingRules->fieldID;
			$POSMTypeID= $iWantVotingRules->fieldID;
			$price_rangeID = $iWantVotingRules->price_rangeID;
			$val 	   = $iWantVotingRules->val;
			$rel 	   = $iWantVotingRules->rel;
			$cond1 			  = $iWantVotingRules->cond1;
			$min_val 		  = $iWantVotingRules->min_val;
			$logical_operator = $iWantVotingRules->logical_operator;
			$cond2   		  = $iWantVotingRules->cond2;
			$max_val 		  = $iWantVotingRules->max_val;
			$action = HTTP_PATH."users/iWantVotingRules/update/$iWantVotingRulesID";
		}
...
<?php 
	...
	echo $CI->forms->form_header('SMBi','iWantVotingRules',$action);
...
?>
		...
		<div class='fl  searhPanel' style='width:170%;margin-top:-20px;'>
			  ...
			  <select name='countryID' style='width:25%;margin-right:10px;' disabled>
			   ...
			   ?>
			   <!-- POSMTypeID -->
			   <?php if(!isset($iWantVotingRulesID) OR (isset($iWantVotingRulesID) AND $fieldName=='POSMTypeID')){ ?>
			   ...
			   <select name='cond1' class='fl' style='width:10%;margin-right:26px;'>
				...
				<?php
				if(isset($POST) AND !isset($iWantVotingRulesID)){ 
				extract($POST);
				$val 	 = "";
				$fieldID = "";
				$rel     = "";
