<div class="content">
	...
	<div class="working_area">
		...
		<div class='fl ' style='width:100%;margin-top:-12px;'>
		    <form name='BU_activeness' style="margin: 10px 10px 10px 10px;" action='<?php echo HTTP_PATH ."report2/price_range_summary" ?>' method='post'>
			<?php 
			$opt = array(
						array('val'=>'cName',   		'label'=>'Country'),
						array('val'=>'price_level',   	'label'=>'Price Label'),
		  ...
		  <?php
		   if($limit+2 <= $totrec) 
			...
			?>
			<?php echo "<div class='fr' style='margin: -2px 1px 8px 0px;'> Record: ". $value .' of <b>' . $totrec ."</b>"; ?> 
			<select onchange="document.forms[0].submit()" name='selpage' style="font-size:12px;width:98px;"> 
			  <?php 
				
				for($x=0; $x <= $totrec-1;$x+=2)
				   {
					if($x+2 <= $totrec) 
					  $value = ($x+1) .'-'. ($x+2);
					else
			...
			<div class="cl"></div>
			<input type='submit' name='Submit' value='Submit' style='margin-left: 83.2%;width:80px;margin-top: -8px;' class='Button_Under_Report'>
			<input type='submit' name='Reset' value='Reset' style='margin-left: 5px;width:80px;margin-top: -8px;' class='Button_Under_Report'>
