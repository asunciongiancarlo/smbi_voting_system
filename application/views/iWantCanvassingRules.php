<div class="content">
		...
		<?php
			...
			{extract($country);
				...
				{ extract($p_type);
				  ...
				  $sql = $CI->db->query("SELECT COUNT(id) as num_of_levels_present FROM iWantCanvassingRules WHERE fieldID = $pID AND countryID = $cID");
				  $sql = $sql->row();
				  
				  $img = "";
				  
				  if($ADD AND ($sql1->num_of_level!=$sql->num_of_levels_present)) 
					$img = "<a href='".HTTP_PATH."users/iWantCanvassingRules/add/$cID/$pID' style='color:white;'>
								<img src='".HTTP_PATH."img/add-item.png' style='margin-left:21px;cursor:pointer;'> Add Voting Rules 
							</a>";
				  echo "<tr>    
						<td colspan='8' class='Itype'  style='text-align:left;background-color:#A5A5A5;color:white;' class='alter'><b>Item Type: $typeName</b></td> 
						<td class='Itype' style='text-align:left;width: 172px;background-color:#A5A5A5;color:white;' class='alter'> $img </td>  
					 </tr>";
				  //SELECT VOTING RULES
				  $CI->load->database('default');
				  $sql = $CI->db->query("SELECT i.id as iWantCanvassingRulesID, xOrder, level_name, campaign_label, rel, val,  
										i.cond1 AS icond1, i.min_val as imin_val, i.logical_operator as ilogical_operator, i.cond2 as icond2, i.max_val as imax_val
										FROM iWantCanvassingRules as i 
				  ...
				  { extract($s);
					...
					if($EDIT)
						echo "<a href='".HTTP_PATH."users/iWantCanvassingRules/edit/$iWantCanvassingRulesID'>Edit</a> | ";
					if($DELETE)	
						echo "<a style='cursor:pointer' onclick='deleteOneItem($iWantCanvassingRulesID)'>Delete</a> </td>";
					echo "</tr>";
				  }
				}
	...
	<?php if($last>0){ ?>
		<ul class="pagination">
		<a href="<?php echo HTTP_PATH."users/iWantCanvassingRules/page/1" ?>"><li class="page-btn" style="margin-left:3px;"> &laquo; FIRST </li></a> 
		<?php 
			//PAGNINATION
			$i	   = 1; 
			$page  = 1;
			...
			while($l!=0)
			{
				...
				//ACTIVE PAGE
				$style="";
				$page_link = HTTP_PATH."users/iWantCanvassingRules/page/".$i++;
				if($active_page==$page++)
				{
		...
		?>
		<a href="<?php echo HTTP_PATH."users/iWantCanvassingRules/page/".$last ?>"><li class="page-btn">LAST &raquo;</li></a>
		</ul>
	<?php } ?>
    </div> 
...
<script type="text/javascript">
	...
	{
		jConfirm("Are you sure you want to delete this iWant Canvassing Rule?","Alert",function(r){
			if(r) window.location = "<?php echo HTTP_PATH ?>users/iWantCanvassingRules/deleteOneItem/"+ id;
		});
