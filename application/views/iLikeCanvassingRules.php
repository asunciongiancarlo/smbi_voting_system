<div class="content">
	...
	<div class="f title-content">
		<h2><span style='text-transform:lowercase;'>i</span>Like Canvassing Rules</h2>
	</div>
	...
	<div class="working_area">
		...
		<?php
			...
			{extract($country);
				...
				echo "<tr>   
						<td class='country-name'  colspan='8' style='text-align:left;background-color:#330404;color:white;' class='alter'><b>Country: $countryName</b></td> 
				...
				{ extract($p_type); 
				  ...
				  $sql = $CI->db->query("SELECT COUNT(id) as num_of_levels_present FROM iLikeCanvassingRules WHERE fieldID = $pID AND countryID = $cID");
				  $sql = $sql->row();
				  
				  $img = "";
				  if($ADD AND ($sql1->num_of_level!=$sql->num_of_levels_present)) 
					$img = "<a href='".HTTP_PATH."users/iLikeCanvassingRules/add/$cID/$pID' style='color:white;'>
				  ...
				  //SELECT VOTING RULES
				  $sql = $CI->db->query("SELECT i.id as iLikeCanvassingRulesID, xOrder, level_name, campaign_label, rel, val,  
										i.cond1 AS icond1, i.min_val as imin_val, i.logical_operator as ilogical_operator, i.cond2 as icond2, i.max_val as imax_val
										FROM iLikeCanvassingRules as i 
				  ...
				  { extract($s);
					...
					if($EDIT)
						echo "<a href='".HTTP_PATH."users/iLikeCanvassingRules/edit/$iLikeCanvassingRulesID'>Edit</a> | ";
					if($DELETE)	
						echo "<a style='cursor:pointer' onclick='deleteOneItem($iLikeCanvassingRulesID)'>Delete</a> </td>";
					echo "</tr>";
				  }
				}
	...
	<?php if($last>0){ ?>
		<ul class="pagination">
		<a href="<?php echo HTTP_PATH."users/iLikeCanvassingRules/page/1" ?>"><li class="page-btn" style="margin-left:3px;"> &laquo; FIRST </li></a> 
		<?php 
			//PAGNINATION
			$i	   = 1; 
			$page  = 1;
			...
			while($l!=0)
			{
				...
				//ACTIVE PAGE
				$style="";
				$page_link = HTTP_PATH."users/iLikeCanvassingRules/page/".$i++;
				if($active_page==$page++)
				{
		...
		?>
		<a href="<?php echo HTTP_PATH."users/iLikeCanvassingRules/page/".$last ?>"><li class="page-btn">LAST &raquo;</li></a>
		</ul>
	<?php } ?>
    </div> 
...
<script type="text/javascript">
	...
	{
		jConfirm("Are you sure you want to delete this iLike Canvassing Rule?","Alert",function(r){
			if(r) window.location = "<?php echo HTTP_PATH ?>users/iLikeCanvassingRules/deleteOneItem/"+ id;
		});
